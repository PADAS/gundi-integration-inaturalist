...
async def test_execute_pull_observations_action(
        ...
        mock_get_gundi_api_key, mock_gundi_sensors_client_class,
        mock_get_observations_v2, mock_publish_event, mock_gundi_client_v2_class
):
    mock_gundi_client_v2.get_integration_details.return_value = async_return(inaturalist_integration_v2)
    ...
    assert response["result"].get("events_extracted")
...
async def test_execute_pull_observations_action_without_bounding_box(
    ...
    mock_gundi_client_v2.get_integration_details.return_value = async_return(inaturalist_integration_v2_without_bounding_box)
    mocker.patch("app.services.action_runner._portal", mock_gundi_client_v2)
    mocker.patch("app.services.activity_logger.publish_event", mock_publish_event)
    mocker.patch("app.services.action_runner.publish_event", mock_publish_event)
    mocker.patch("app.actions.handlers.state_manager", mock_state_manager)
